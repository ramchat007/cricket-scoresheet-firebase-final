  ...
  async function handleBall(code, extraData = {}, physicalRuns = 0) {
    ...
    try {
      await ballTransaction(tournamentId, matchId, (s) => {
        ...
        if (isWD)
          displayCode = `1wd${physicalRuns > 0 ? "+" + physicalRuns : ""}`;
        if (isNB)
          displayCode = `1nb${physicalRuns > 0 ? "+" + physicalRuns : ""}`;
        ...
        let shouldSwap =
          (physicalRuns || (code !== "W" && parseInt(code))) % 2 !== 0;
  ...
  return {
    ...
    handleStrikeChange: (s, ns) =>
      ballTransaction(tournamentId, matchId, (st) => {
        st.innings[st.currentInnings].striker = s;
        st.innings[st.currentInnings].nonStriker = ns;
        return st;
      }),
    handleEndInnings: () =>
      ballTransaction(tournamentId, matchId, (s) => {
