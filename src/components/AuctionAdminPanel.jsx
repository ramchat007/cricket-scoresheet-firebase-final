  ...
  const handleClearAuctionPool = async () => {
    ...
    try {
      ...
      batch.update(stateRef, {
        ...
        status: "PAUSED", // Pause the auction so it doesn't auto-start
        activeSlotId: null, // Clear the selected slot
      });

      await batch.commit();
      alert("Auction pool completely cleared and stage reset!");
    } catch (e) {
      console.error(e);
