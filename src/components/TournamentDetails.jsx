    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-gray-900 border border-gray-700 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            <div className="text-center text-cyan-500 animate-pulse mt-4">Loading Global Database...</div>
          ) : filteredTeams.length === 0 ? (
            <div className="text-center text-gray-500 mt-4">No available teams found.</div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer p-3 rounded-xl border flex items-center justify-between transition-all ${
                      isSelected ? "bg-cyan-900/20 border-cyan-500 shadow-md" : "bg-gray-800 border-gray-700 hover:border-gray-500"
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      {team.logo ? (
                        ...
                        <div className="w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center text-lg">🛡️</div>
                      )}
                      <div>
                        <div className="font-bold text-white text-sm">{team.name}</div>
                        <div className="text-sm text-gray-400">{team.players ? team.players.length : 0} Players</div>
                      </div>
                    </div>
                    {isSelected && <div className="text-cyan-400 text-xl">✓</div>}
                  </div>
                );
              })}
            </div>
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-950 flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-gray-400 hover:bg-gray-800 transition-colors text-sm font-bold">Cancel</button>
          <button
            disabled={selectedTeams.length === 0}
            onClick={handleImport}
            className={`px-6 py-2 rounded-lg text-white font-bold text-sm transition-all ${
              selectedTeams.length > 0 ? "bg-gradient-to-r from-cyan-600 to-blue-600 hover:shadow-lg" : "bg-gray-700 cursor-not-allowed opacity-50"
            }`}
          >
...
export default function TournamentDetails() {
  ...
  const isOwner = isAdmin || (user && tournamentData && user.uid === tournamentData.ownerId);
  ...
  const renderMatchCard = (m) => {
    const status = m.status || m.meta?.status || "upcoming";
    const normStatus = status.toLowerCase();
    const isLive = normStatus === "in-progress" || normStatus === "ongoing" || normStatus === "live";
    ...
    return (
      ...
      >
        <div className={`h-1 w-full ${isLive ? "bg-red-500 animate-pulse" : isFinished ? "bg-green-500" : "bg-blue-500"}`}></div>
        <div className="p-5">
          <div className="flex justify-between items-start mb-4">
            ...
            {isLive && (
              <span className="text-[10px] font-bold text-red-400 bg-red-900/20 px-2 py-1 rounded border border-red-900/30 animate-pulse">LIVE</span>
            )}
            {isFinished && (
              <span className="text-[10px] font-bold text-green-400 bg-green-900/20 px-2 py-1 rounded border border-green-900/30">FINISHED</span>
            )}
          </div>
          <div className="flex flex-col gap-2 mb-4">
            <div className="text-lg font-bold text-white group-hover:text-cyan-400 transition-colors">
              {m.meta?.teamA} <span className="text-gray-600 text-sm mx-1">vs</span> {m.meta?.teamB}
            </div>
            {isFinished && (
              <div className="text-sm text-green-400 flex items-center gap-1">
                🏆 Won by {m.winner || m.result?.winner || m.meta?.result || "Unknown"}
              </div>
            )}
          </div>
          {canEdit && (
            <div className="mt-4 pt-3 border-t border-gray-800 flex justify-end">
              <button
                ...
                className={`text-sm font-bold px-4 py-2 rounded transition-all uppercase tracking-wider flex items-center gap-2 shadow-lg ${
                  isLive
                    ? "bg-red-600 hover:bg-red-500 text-white shadow-red-900/30 animate-pulse"
                    : isFinished
              ...
              >
                {isLive ? <>🔴 Resume Scoring</> : isFinished ? <>📝 Edit Record</> : <>⚡ Start Scoring</>}
              </button>
            </div>
          )}
        </div>
  ...
  const pointsTable = useMemo(() => {
    ...
    const mergedTable = tournamentTeams.map((team) => {
      ...
      return { name: teamName, played: 0, won: 0, lost: 0, points: 0, nrr: "0.000" };
    });
  ...
  const detailedStats = useMemo(() => {
    ...
    return Object.values(players).map((p) => {
      ...
      const bowlEco = oversBowled > 0 ? (p.runsConceded / oversBowled).toFixed(2) : "0.00";
  ...
  const filteredStats = useMemo(() => {
    ...
    return data.sort((a, b) => {
      if (statsTab === "bat") {
        ...
        if (sortStyle === "average") return parseFloat(b.batAvg) - parseFloat(a.batAvg);
        ...
        if (sortStyle === "best_avg") return parseFloat(a.bowlAvg) - parseFloat(b.bowlAvg);
  ...
  const distinctTeams = useMemo(() => [...new Set(detailedStats.map((p) => p.team).filter(Boolean))], [detailedStats]);

  const getUnifiedHistory = (history) => {
    const unified = {};
    history.forEach((h) => {
      if (!unified[h.matchId]) unified[h.matchId] = { ...h, bat: null, bowl: null, totalPoints: 0 };
      if (h.type === "bat") unified[h.matchId].bat = h;
      if (h.type === "bowl") unified[h.matchId].bowl = h;
      unified[h.matchId].totalPoints += h.mvpPoints || 0;
    });
    return Object.values(unified).sort((a, b) => new Date(b.date) - new Date(a.date));
  };

  if (loading) return <div className="flex justify-center items-center min-h-screen bg-gray-950 text-cyan-500 animate-pulse font-bold">LOADING...</div>;
  if (!tournamentData) return <div>Not Found</div>;

  return (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 min-h-screen">
      <GlobalTeamSelector isOpen={showImportModal} onClose={() => setShowImportModal(false)} onImport={handleImportTeams} existingTeamIds={tournamentTeams.map((t) => t.originalTeamId)} />

      {/* HEADER */}
      <div className="mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <div className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-1">Tournament Dashboard</div>
            <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-tight">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-cyan-400 to-green-400">{tournamentData.name || id}</span>
            </h2>
          ...
          <div className="bg-gray-800 p-1 rounded-lg flex items-center border border-gray-700 overflow-x-auto">
            {[{ id: "matches", label: "Matches" }, { id: "points", label: "Points" }, { id: "players", label: "Stats" }, { id: "admin", label: "Admin" }]
              .filter((tab) => tab.id !== "admin" || isOwner)
              .map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`px-4 py-2 rounded-md text-sm font-bold whitespace-nowrap transition-all ${activeTab === tab.id ? "bg-gray-700 text-white shadow-sm border border-gray-600" : "text-gray-400 hover:text-gray-200"}`}
                >
                  {tab.label}
                </button>
              ))}
      ...
      {activeTab === "matches" && (
        <div className="animate-in fade-in space-y-10">
          {liveMatches.length > 0 && (
            <div>
              <h3 className="text-white font-bold text-xl mb-4 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span>Live Matches</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{liveMatches.map(renderMatchCard)}</div>
            </div>
          )}
          <div>
            <h3 className="text-white font-bold text-xl mb-4 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-blue-500"></span>Upcoming Matches</h3>
            {upcomingMatches.length === 0 ? <div className="text-gray-600 italic text-sm">No upcoming matches scheduled.</div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{upcomingMatches.map(renderMatchCard)}</div>}
          </div>
          {finishedMatches.length > 0 && (
            <div>
              <h3 className="text-white font-bold text-xl mb-4 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-green-500"></span>Finished Matches</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{finishedMatches.map(renderMatchCard)}</div>
            </div>
          )}
          {matches.length === 0 && <div className="text-gray-500 text-center py-20 bg-gray-900/50 rounded-xl border border-dashed border-gray-800">No matches found for this tournament.</div>}
        </div>
      )}
              ...
              <tbody className="divide-y divide-gray-800">
                {pointsTable.map((t, i) => (
                  <tr key={i} className="hover:bg-gray-800/50">
                    ...
                    <td className="px-4 text-center text-xl font-black text-cyan-400">{t.points}</td>
                    <td className="px-4 text-right">{t.nrr}</td>
                  </tr>
                ))}
                {pointsTable.length === 0 && <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500 italic">No teams registered yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            {orangeCap && (
              <div className="bg-gradient-to-br from-orange-900/30 to-gray-900 border border-orange-500/30 p-4 rounded-xl flex items-center gap-4 relative overflow-hidden shadow-lg shadow-orange-900/10">
                ...
                <div className="bg-orange-500/20 text-orange-400 p-3 rounded-full text-2xl border border-orange-500/40">🧢</div>
                <div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-orange-500">Orange Cap</div>
                  <div className="text-xl font-bold text-white leading-tight">{orangeCap.name}</div>
                  <div className="text-sm text-gray-400">{orangeCap.runs} Runs</div>
                </div>
              </div>
            )}
            {purpleCap && (
              <div className="bg-gradient-to-br from-purple-900/30 to-gray-900 border border-purple-500/30 p-4 rounded-xl flex items-center gap-4 relative overflow-hidden shadow-lg shadow-purple-900/10">
                <div className="absolute top-0 right-0 p-4 opacity-10 text-6xl">🥎</div>
                <div className="bg-purple-500/20 text-purple-400 p-3 rounded-full text-2xl border border-purple-500/40">🧢</div>
                <div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-purple-500">Purple Cap</div>
                  <div className="text-xl font-bold text-white leading-tight">{purpleCap.name}</div>
                  <div className="text-sm text-gray-400">{purpleCap.wickets} Wickets</div>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
            {["bat", "bowl", "mvp"].map((type) => (
              <button
                ...
                onClick={() => {
                  setStatsTab(type);
                  setSortStyle(type === "bat" ? "most_runs" : type === "bowl" ? "most_wickets" : "mvp");
                }}
                className={`flex-1 min-w-[80px] py-2 rounded-lg font-bold text-sm uppercase tracking-wider transition-all ${
                  statsTab === type ? "bg-cyan-600 text-white shadow-lg" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
                }`}
              >
                {type === "bat" ? "🏏 Batting" : type === "bowl" ? "🥎 Bowling" : "👑 MVP"}
              </button>
            ))}
          </div>
              ...
              <div key={index} className="border-b border-gray-800 last:border-0">
                <div
                  className={`p-4 flex items-center justify-between cursor-pointer transition-colors ${
                    expandedPlayer === p.name ? "bg-gray-800" : "hover:bg-gray-800/30"
                  }`}
                  onClick={() => setExpandedPlayer(expandedPlayer === p.name ? null : p.name)}
                >
                  <div className="flex items-center gap-3">
                    <div className="text-xl font-black text-gray-600 w-6">{(index + 1).toString().padStart(2, "0")}</div>
                    <div>
                  ...
                  <div className="text-right">
                    {statsTab === "bat" && (
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          ...
                          <div className="font-bold text-cyan-400">{p.runs}</div>
                        </div>
                        <div>
                          <div className="text-[10px] text-gray-500 uppercase">Inn</div>
                    ...
                    {statsTab === "bowl" && (
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <div className="text-[10px] text-gray-500 uppercase">Wkts</div>
                          <div className="font-bold text-cyan-400">{p.wickets}</div>
                        </div>
                        <div>
                    ...
                    {statsTab === "mvp" && (
                      <div className="flex flex-col items-end">
                        <div className="text-xl font-black text-yellow-500">
                          {p.mvp} <span className="text-[10px] text-gray-500 font-normal">pts</span>
                        </div>
                ...
                {/* --- EXPANDED STATS DETAIL VIEW --- */}
                {expandedPlayer === p.name && (
                  <div className="bg-gray-950/50 p-4 border-t border-gray-800 animate-in slide-in-from-top-2">
                    
                    <div className="flex flex-col gap-3 mb-4 pb-4 border-b border-gray-800/50">
                      
                      {/* 1. BATTING ROW (Show if Bat Tab OR MVP with Runs) */}
                      {(statsTab === 'bat' || (statsTab === 'mvp' && p.runs > 0)) && (
                        <div className="grid grid-cols-4 gap-2 text-center text-sm">
                          <div className="bg-gray-900 p-2 rounded">Runs: <b className="text-cyan-400">{p.runs}</b></div>
                          <div className="bg-gray-900 p-2 rounded">4s: <b className="text-green-400">{p.fours}</b></div>
                          <div className="bg-gray-900 p-2 rounded">6s: <b className="text-yellow-400">{p.sixes}</b></div>
                          <div className="bg-gray-900 p-2 rounded">Avg: <b className="text-white">{p.batAvg}</b></div>
                        </div>
                      )}

                      {/* 2. DIVIDER FOR MVP */}
                      {statsTab === 'mvp' && p.runs > 0 && p.ballsBowled > 0 && (
                        <div className="border-t border-gray-800/30"></div>
                      )}

                      {/* 3. BOWLING ROW (Show if Bowl Tab OR MVP with Overs) */}
                      {(statsTab === 'bowl' || (statsTab === 'mvp' && p.ballsBowled > 0)) && (
                        <div className="grid grid-cols-4 gap-2 text-center text-sm">
                          <div className="bg-gray-900 p-2 rounded">Ov: <b className="text-white">{(p.ballsBowled / 6).toFixed(1)}</b></div>
                          <div className="bg-gray-900 p-2 rounded">Runs: <b className="text-red-400">{p.runsConceded}</b></div>
                          <div className="bg-gray-900 p-2 rounded">Wkts: <b className="text-cyan-400">{p.wickets}</b></div>
                          <div className="bg-gray-900 p-2 rounded">Eco: <b className="text-gray-300">{p.bowlEco}</b></div>
                        </div>
                      )}
                    </div>

                    <div className="space-y-2">
                      <div className="text-[10px] font-bold text-gray-500 uppercase mb-2">Match History</div>
                      {(statsTab === "mvp" ? getUnifiedHistory(p.history) : p.history.filter((h) => h.type === statsTab)).map((log, lIdx) => (
                        <div key={lIdx} onClick={() => navigate(`/tournaments/${id}/scorecard/${log.matchId}`)} className="flex justify-between items-center bg-gray-900 hover:bg-gray-800 p-2 rounded cursor-pointer transition-colors">
                          <div className="text-sm">
                            <div className="text-gray-400">{log.date}</div>
                            <div className="font-bold text-gray-200">vs {log.opponent}</div>
                          </div>
                          <div className="text-sm font-mono text-right">
                            {statsTab === "bat" && (
                              <>
                                <span className={`font-bold ${log.runs >= 30 ? "text-yellow-400" : "text-white"}`}>{log.runs}</span>
                                <span className="text-gray-500 text-[10px]">({log.balls})</span>
                                {log.isNotOut && <span className="text-cyan-400 text-[10px] ml-1">*</span>}
                              </>
                            )}
                            {statsTab === "bowl" && (
                              <>
                                <span className={`font-bold ${log.wickets >= 2 ? "text-yellow-400" : "text-white"}`}>{log.wickets}</span>
                                <span className="text-gray-500 mx-1">/</span>
                                <span className="text-gray-400">{log.runsConceded}</span>
                                <div className="text-[10px] text-gray-500">({log.overs} ov)</div>
                              </>
                            )}
                            {statsTab === "mvp" && (
                              <div className="flex flex-col items-end">
                                <span className="font-bold text-yellow-400">{log.totalPoints} pts</span>
                                <div className="text-[10px] text-gray-500 flex gap-2">
                                  {log.bat && <span>🏏 {log.bat.runs}({log.bat.balls})</span>}
                                  {log.bowl && <span>🥎 {log.bowl.wickets}/{log.bowl.runsConceded}</span>}
                                </div>
                              </div>
                            )}
                          </div>
                      ...
                      {p.history.length === 0 && <div className="text-sm text-gray-500 italic">No activity in this category.</div>}
                    </div>
                  </div>
                )}
              </div>
            ))}
            {filteredStats.length === 0 && <div className="p-8 text-center text-gray-500">No stats available for this selection.</div>}
          </div>
        </div>
      )}
      ...
      {canEdit && activeTab === "admin" && (
        <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 shadow-lg">
            <h3 className="text-white font-bold text-lg mb-2">🛡️ Team Management</h3>
            <button onClick={() => setShowImportModal(true)} className="w-full bg-blue-600/20 text-blue-400 border border-blue-600/30 px-4 py-2 rounded-lg font-bold mb-4">+ Import Teams</button>
            <TeamManager tournamentId={id} />
          </div>
          {isOwner && (
            <div className="space-y-4">
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 shadow-lg">
                <h3 className="text-white font-bold text-lg mb-2">🔐 Access Control</h3>
                <TournamentAccessManager tournament={tournamentData} currentUserId={user.uid} />
              </div>
              <div className="bg-red-950/20 border border-red-900/50 rounded-xl p-5">
                ...
                <button onClick={handleDeleteTournament} className="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded font-bold w-full transition-colors">Delete Tournament</button>
              </div>
            </div>
          )}
        </div>
