    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="bg-gray-900 border border-gray-700 w-full max-w-md rounded-xl shadow-2xl flex flex-col max-h-[80vh]">
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between p-2 rounded cursor-pointer ${
                  isSel ? "bg-cyan-900/40 border border-cyan-500/50" : "hover:bg-gray-800"
                }`}
              >
        ...
        <div className="p-3 border-t border-gray-800 flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 text-gray-400 font-bold text-sm">Cancel</button>
          <button
...
export default function MatchSetup({
  ...
  const [availableTournaments, setAvailableTournaments] = useState(initialAvailableTournaments || []);
  ...
  const handlePlayersPicked = (pickedPlayers) => {
    const newRosterItems = pickedPlayers.map(p => ({
      id: p.id, name: p.name, role: p.role, isGuest: false
    }));
  ...
  const getSmartSquad = (textInput, roster) => {
    ...
    return names.map(name => {
      ...
      return { id: `guest_${Date.now()}_${Math.random()}`, name, isGuest: true };
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!exists) {
        await addTournament(tournament, { 
            name: tournament, createdAt: new Date().toISOString(), status: "upcoming" 
        });
    }
    ...
    const payload = {
      meta: {
        tournament, teamA, teamB, overs: Number(overs),
        createdAt: new Date().toISOString(), status: "upcoming",
        date: tournamentDate, format: tournamentFormat
      },
      teamASquad: squadA,
      teamBSquad: squadB,
      innings: []
  ...
  // Styles
  const inputClass = "w-full bg-gray-800 text-white border border-gray-700 rounded-lg px-3 py-3 focus:outline-none focus:border-cyan-500 transition-all";
  const labelClass = "block text-gray-400 text-sm font-bold mb-1 uppercase tracking-wider";
    ...
    <div className="max-w-4xl mx-auto mb-10">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="bg-gray-950 p-1 flex border-b border-gray-800">
            <button onClick={() => setActiveTab("single")} className={`flex-1 py-3 text-sm font-bold uppercase ${activeTab === "single" ? "bg-gray-800 text-white" : "text-gray-500"}`}>Single Match</button>
            <button onClick={() => setActiveTab("auto")} className={`flex-1 py-3 text-sm font-bold uppercase ${activeTab === "auto" ? "bg-cyan-900/20 text-cyan-400" : "text-gray-500"}`}>Auto Schedule</button>
        </div>
        ...
        <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div>
                    ...
                    <input list="tList" value={tournament} onChange={e => setTournament(e.target.value)} className={inputClass} placeholder="Select/Type Name" />
                    <datalist id="tList">{availableTournaments.map(t => <option key={t.id} value={t.id} />)}</datalist>
                </div>
                <div>
            ...
            {activeTab === "single" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in">
                    {/* TEAM A */}
                    <div>
                        <label className={`${labelClass} text-cyan-400`}>Team A Name</label>
                        <input value={teamA} onChange={(e) => handleTeamChange(e, setTeamA, setBatsmenText, setTeamARoster)} className={inputClass} list="teamList" placeholder="e.g. India" />
                        <datalist id="teamList">{teams.map(t => <option key={t.id} value={t.name} />)}</datalist>
                        
                        <div className="flex justify-between items-center mt-3 mb-1">
                            <label className={labelClass}>Squad</label>
                            <button onClick={() => openPicker("A")} className="text-xs bg-gray-800 px-2 py-1 rounded text-cyan-400 hover:text-white border border-gray-700">
                                🌍 Pick Players
                            </button>
                        </div>
                        <textarea value={batsmenText} onChange={e => setBatsmenText(e.target.value)} className={`${inputClass} h-24 text-sm font-mono`} placeholder="Type names (comma) or pick..." />
                    </div>

                    {/* TEAM B */}
                    <div>
                        <label className={`${labelClass} text-green-400`}>Team B Name</label>
                        <input value={teamB} onChange={(e) => handleTeamChange(e, setTeamB, setBowlersText, setTeamBRoster)} className={inputClass} list="teamList" placeholder="e.g. Australia" />
                        
                        <div className="flex justify-between items-center mt-3 mb-1">
                            <label className={labelClass}>Squad</label>
                            <button onClick={() => openPicker("B")} className="text-xs bg-gray-800 px-2 py-1 rounded text-green-400 hover:text-white border border-gray-700">
                                🌍 Pick Players
                            </button>
                        </div>
                        <textarea value={bowlersText} onChange={e => setBowlersText(e.target.value)} className={`${inputClass} h-24 text-sm font-mono`} placeholder="Type names (comma) or pick..." />
                    </div>

                    <button onClick={handleSubmit} disabled={!teamA || !teamB} className="col-span-1 md:col-span-2 py-4 bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-bold rounded-lg shadow-lg hover:shadow-cyan-500/20 transition-all uppercase tracking-widest mt-4 disabled:opacity-50">
                        + Create Match
                    </button>
                </div>
            )}
            ...
            {activeTab === "auto" && (
                <div className="text-center py-10 text-gray-500">
                    Auto-scheduler uses Team Manager teams. <br/> Please create teams in Dashboard first for bulk scheduling.
                </div>
            )}
        </div>
      </div>
