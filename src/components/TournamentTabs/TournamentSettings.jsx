...
export default function TournamentSettings({
  ...
  const [formData, setFormData] = useState({
    ...
    endDate: "",
    maxPlayers: "",
    status: "upcoming", // 🟢 ADDED STATUS STATE
  });
  ...
  useEffect(() => {
    ...
    let derivedStatus = tournament.status || "upcoming";
    if (
      ["completed", "finished"].includes(derivedStatus.toLowerCase()) ||
      allMatchesFinished
    ) {
    ...
    setFormData({
      ...
      status: derivedStatus, // 🟢 Uses the smart auto-checked status
    });
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await updateDoc(tRef, {
        ...
        status: formData.status, // 🟢 SAVE STATUS OVERRIDE TO DATABASE
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      <form
        ...
        {/* 🟢 ADDED TOURNAMENT STATUS OVERRIDE 🟢 */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label
              className={`flex items-center gap-1.5 text-[10px] font-bold uppercase mb-1 ${theme.sub}`}>
              <Activity
                size={12}
                className={
            ...
            <select
              ...
              <option value="completed">Completed (Closes Tournament)</option>
            </select>
          </div>
        </div>
