...
const ScoreTable = ({
  ...
  const formatDismissal = (stats, isStriker, isNonStriker) => {
    ...
    if (!stats || !stats.out)
      return <span className="text-gray-600 text-[10px] uppercase">DNB</span>;
    ...
    if (stats.wicketType) {
      ...
      switch (stats.wicketType) {
        ...
        default:
          return <span className={style}>{stats.wicketType}</span>;
      }
    }
    return <span className={style}>{stats.out}</span>;
  };
  ...
  const renderInnings = (inn, idx) => {
    ...
    // --- 1. BATTING LOGIC (ORDERED) ---
    // Use explicit battingOrder if available, otherwise fallback to squad list
    const order =
      inn.battingOrder && inn.battingOrder.length > 0
        ? inn.battingOrder
        : inn.batsmenList || [];

    // Get stats keys to check if anyone played who isn't in squad/order list (edge case)
    const statsKeys = inn.batsmenStats ? Object.keys(inn.batsmenStats) : [];

    // Combine to ensure no one is missed
    const combinedPool = [...order, ...statsKeys];
    if (inn.striker) combinedPool.push(inn.striker);
    if (inn.nonStriker) combinedPool.push(inn.nonStriker);

    // Filter Unique & Remove Falsy
    const uniqueBatters = Array.from(new Set(combinedPool)).filter(Boolean);
    ...
    uniqueBatters.forEach((name) => {
      ...
      const isCurrent = name === inn.striker || name === inn.nonStriker;

      // If they have stats OR are currently batting OR are in the explicit battingOrder list up to current point
    ...
    playedBatsmen.sort((a, b) => {
      const idxA = inn.battingOrder ? inn.battingOrder.indexOf(a) : -1;
      const idxB = inn.battingOrder ? inn.battingOrder.indexOf(b) : -1;
    ...
    // --- 2. BOWLING LOGIC (UPDATED) ---
    // Fix: Only show bowlers who have stats AND have actually bowled (balls > 0) OR are current.
    // This prevents showing the entire squad if they were initialized with empty stats.
    const rawBowlers = Object.keys(inn.bowlerStats || {});

    const activeBowlers = rawBowlers.filter((name) => {
      const s = inn.bowlerStats[name];
      const isCurrent = name === inn.currentBowler;
      // Condition: Is Currently Bowling OR Has bowled at least 1 ball
      return isCurrent || (s && s.balls > 0);
    });

    // Optional: Sort active bowlers so current bowler is visually distinct or last?
    // For now we trust the insertion order or default sort.
      ...
      <div
        ...
        <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 p-4 border-b border-gray-700 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold text-gray-500 uppercase tracking-widest block mb-0.5">
                Innings {idx + 1}
              </span>
        ...
        <div className="flex-1 flex flex-col">
              ...
              <tbody className="divide-y divide-gray-800/50">
                {playedBatsmen.length === 0 ? (
                  ...
                  playedBatsmen.map((name) => {
                    const stats = (inn.batsmenStats &&
                      inn.batsmenStats[name]) || {
                      runs: 0,
                      balls: 0,
                      fours: 0,
                      sixes: 0,
                    ...
                    const isNon = name === inn.nonStriker;
                    const sr =
                      stats.balls > 0
                    ...
                    if (isStriker) {
                      rowClass +=
                        " bg-gradient-to-r from-cyan-900/10 to-transparent border-l-2 border-cyan-500";
                    ...
                    return (
                      <tr key={`bat-${name}`} className={rowClass}>
                        <td className="px-4 py-3 whitespace-nowrap">
                          ...
                          {isStriker && (
                            <span className="text-cyan-400 ml-2 text-[10px] align-top animate-pulse">
                              ★
                            </span>
                          )}
                        </td>
          ...
          {dnbBatsmen.length > 0 && (
            <div className="bg-gray-950/30 px-4 py-3 border-t border-gray-800 text-[11px] flex gap-2 overflow-x-auto">
              <span className="font-bold text-gray-600 uppercase whitespace-nowrap shrink-0">
                Yet to Bat:
              </span>
          ...
          <div className="bg-gray-950/20 border-t border-gray-800 mt-auto">
            <div className="px-4 py-2 text-[10px] font-bold text-cyan-600 uppercase tracking-widest border-b border-gray-800 bg-gray-900/40">
              Bowling
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left border-collapse">
                    ...
                    activeBowlers.map((b) => {
                      ...
                      const econ =
                        s.balls > 0
                          ...
                          : "0.0";
                      const isCurrent = b === inn.currentBowler;
                      return (
                        <tr
                          key={`bowl-${b}`}
                          className={
                          ...
                          <td className="px-2 py-2.5 text-center font-mono text-white font-medium">
                            {overStr}
                          </td>
      ...
      {isFinished && mom && (
        <div className="mb-8 bg-gradient-to-r from-yellow-900/40 via-gray-900 to-gray-900 border border-yellow-500/30 rounded-xl p-5 flex items-center justify-between shadow-lg shadow-yellow-900/20 relative overflow-hidden">
          ...
          <div className="flex items-center gap-5 relative z-10">
            <div className="w-14 h-14 bg-yellow-500/20 rounded-full flex items-center justify-center text-3xl border border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.3)]">
              🏅
            </div>
            <div>
