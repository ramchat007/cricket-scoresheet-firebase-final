...
import { listGlobalPlayers, createGlobalPlayer, updateGlobalPlayer } from "../utils/firestore";
...
export default function GlobalPlayersView() {
  ...
  // Sorting State
  const [sortConfig, setSortConfig] = useState({ key: "runs", direction: "desc" });
  ...
  const handleSort = (key) => {
    let direction = "desc"; // Default to desc for stats
  ...
  const sortedPlayers = useMemo(() => {
    ...
    if (sortConfig.key) {
      sortable.sort((a, b) => {
        ...
        // Check if sorting by top-level prop or nested stat
  ...
  // 3. Image Compression (Kept same)
  const handleImageUpload = (event) => {
    const file = event.target.files[0];
    if (!file) return;
    if (file.size > .2 * 900 * 900) return alert("Image too large (Max 200KB)");
  ...
  const openEditModal = (player, e) => {
    ...
    // Helper to sanitize "Unknown" values
    ...
    setFormData({
      ...
      // FIX: Force "Unknown" to become "Right Hand Bat" immediately
      battingStyle: sanitizeStyle(player.battingStyle, "Right Hand Bat"),
      
      // FIX: Force "Unknown" to become "Right Arm Medium" immediately
  ...
  const SortIcon = ({ colKey }) => {
    if (sortConfig.key !== colKey) return <span className="text-gray-600 ml-1 opacity-0 group-hover:opacity-50">⇅</span>;
    return <span className="text-cyan-400 ml-1">{sortConfig.direction === "asc" ? "↑" : "↓"}</span>;
  };

  // Helper for Navigating to Match
    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-2 md:p-4 pb-20">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-2xl">
          {loading ? (
            <div className="p-12 text-center text-cyan-500 animate-pulse text-sm font-mono">Loading Database...</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left border-collapse">
                <thead className="bg-gray-950 text-gray-400 text-[10px] uppercase font-bold tracking-wider">
                  <tr>
                    {/* COMBINED PLAYER DETAILS COLUMN */}
                    <th className="px-4 py-3 cursor-pointer hover:text-white group w-[30%]" onClick={() => handleSort("name")}>
                      Player Details <SortIcon colKey="name" />
                    </th>
                    
                    {/* STATS COLUMNS */}
                    <th className="px-2 py-3 text-center cursor-pointer hover:text-white group" onClick={() => handleSort("matches")}>
                      Mat <SortIcon colKey="matches" />
                    </th>
                    <th className="px-2 py-3 text-center cursor-pointer hover:text-white group" onClick={() => handleSort("runs")}>
                      Runs <SortIcon colKey="runs" />
                    </th>
                    <th className="px-2 py-3 text-center cursor-pointer hover:text-white group hidden sm:table-cell" onClick={() => handleSort("highestScore")}>
                      HS <SortIcon colKey="highestScore" />
                    </th>
                    <th className="px-2 py-3 text-center cursor-pointer hover:text-white group" onClick={() => handleSort("wickets")}>
                      Wkts <SortIcon colKey="wickets" />
                    </th>
                    <th className="px-2 py-3 text-center cursor-pointer hover:text-white group hidden sm:table-cell" onClick={() => handleSort("bestBowling")}>
                      BB <SortIcon colKey="bestBowling" />
                    </th>
                    <th className="px-2 py-3 text-center hidden md:table-cell">
                      Field <span className="text-[9px] text-gray-600">(Ct/St)</span>
                    </th>
                    ...
                    sortedPlayers.map((player) => (
                      <React.Fragment key={player.id}>
                        <tr 
                          ...
                          className={`hover:bg-gray-800/50 transition-colors cursor-pointer ${expandedPlayerId === player.id ? "bg-gray-800/30" : ""}`}
                        >
                          {/* 1. PLAYER DETAILS (Combined) */}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col">
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                  <span className="text-[9px] bg-gray-800 text-gray-300 px-1.5 rounded border border-gray-700">{player.role}</span>
                                </div>
                          ...
                          {/* 2. STATS */}
                          <td className="px-2 py-3 text-center text-white font-mono">{player.stats?.matches || 0}</td>
                          <td className="px-2 py-3 text-center font-bold text-cyan-400">{player.stats?.runs || 0}</td>
                          <td className="px-2 py-3 text-center text-gray-400 hidden sm:table-cell">{player.stats?.highestScore || 0}</td>
                          <td className="px-2 py-3 text-center font-bold text-green-400">{player.stats?.wickets || 0}</td>
                          <td className="px-2 py-3 text-center text-gray-400 hidden sm:table-cell">{player.stats?.bestBowling || "-"}</td>
                            ...
                            <div className="flex justify-end gap-2">
                              ...
                              {user && (
                                <button 
                                  ...
                                  className="text-gray-500 hover:text-white p-1.5 rounded hover:bg-gray-700 transition-all border border-transparent hover:border-gray-600"
                                  title="Edit Player"
                                >
                                  ✎
                                </button>
                          ...
                          <tr className="bg-gray-950/50">
                            <td colSpan={8} className="p-0">
                              <div className="p-4 border-b border-gray-800 animate-in slide-in-from-top-2">
                                ...
                                {player.stats?.history && player.stats.history.length > 0 ? (
                                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                    {player.stats.history.map((log, idx) => (
                                      <div 
                                        key={idx}
                                        onClick={() => goToMatch(log.tournamentId, log.matchId)}
                                      ...
                                      >
                                        <div className="flex justify-between items-start mb-1">
                                          ...
                                          <span className="text-[10px] text-gray-400 group-hover:text-cyan-400">View ↗</span>
                                        </div>
                                        <div className="font-bold text-sm text-gray-200 mb-1">vs {log.opponent}</div>
                                        <div className="flex gap-3 text-xs">
                                          {log.runs !== undefined && (
                                            <span className={log.runs >= 30 ? "text-yellow-400 font-bold" : "text-gray-400"}>
                                              {log.runs} Runs
                                            </span>
                                          )}
                                          {log.wickets !== undefined && (
                                            <span className={log.wickets >= 2 ? "text-green-400 font-bold" : "text-gray-400"}>
                                              {log.wickets} Wkts
                                            </span>
                                          )}
                                        </div>
                                  ...
                                  <div className="text-sm text-gray-600 italic p-2 border border-dashed border-gray-800 rounded bg-gray-900/50">
                                    ...
                                    <span className="block text-[10px] mt-1 text-gray-700">
                                      (History populates automatically when you finish matches using this player)
                                    </span>
                                  </div>
                                )}
                              </div>
            ...
            <div className="bg-gray-900 border border-gray-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="overflow-y-auto p-6">
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="flex flex-col items-center">
                    <div className="relative group cursor-pointer" onClick={() => fileInputRef.current.click()}>
                      <img
                        ...
                        alt="Player"
                        className={`w-24 h-24 rounded-full border-4 border-gray-800 shadow-lg object-cover bg-gray-700 transition-opacity ${processingImage ? "opacity-50" : "opacity-100"}`}
                      />
                  ...
                  >
                    {processingImage ? "Processing..." : isEditing ? "Update Player" : "Create Player"}
                  </button>
                </form>
              </div>
            </div>
