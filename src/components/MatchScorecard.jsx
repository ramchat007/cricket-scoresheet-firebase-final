...
export default function MatchScorecard() {
  ...
  if (loading)
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-950 text-cyan-500 animate-pulse font-bold tracking-widest uppercase">
        Loading Scorecard...
      </div>
    );
  if (error)
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-950 text-red-500 font-bold">
        {error}
      </div>
    );
  ...
  const InningCard = ({ inningIndex, teamName }) => {
    ...
    const uniqueBatterNames = Array.from(
      new Set(
        allBattersRaw
    ...
    const uniqueBowlerNames = Array.from(
      new Set(
        allBowlersRaw
          .map(cleanName)
    ...
    activeBowlers.sort((a, b) => {
      ...
      if (idxA_Order !== -1 && idxB_Order !== -1)
        return idxA_Order - idxB_Order;
      ...
      if (idxA_Squad !== -1 && idxB_Squad !== -1)
        return idxA_Squad - idxB_Squad;
    ...
    return (
      <div className="mb-8 bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  return (
                    <tr
                      ...
                      className={`transition-colors ${
                        isActive ? "bg-cyan-900/10" : "hover:bg-gray-800/30"
                      }`}>
                      <td
        ...
        <div className="border-t-4 border-gray-900 mt-2">
          <div className="bg-gray-800/50 px-4 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest border-b border-gray-800">
            Bowling
          </div>
