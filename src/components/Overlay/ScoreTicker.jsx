...
export default function ScoreTicker({ match }) {
  ...
  // 🔥 Strict normalization prevents color-bleeding bugs from trailing spaces
  ...
  const defaultTeamAColor = "#0284c7"; 
  const defaultTeamBColor = "#e11d48"; 
  const teamAColor = match.meta?.teamAColor || defaultTeamAColor;
  const teamBColor = match.meta?.teamBColor || defaultTeamBColor;

  const battingColor = isTeamABatting ? teamAColor : teamBColor;
  const bowlingColor = isTeamABatting ? teamBColor : teamAColor;
        ...
        <div 
          ...
          <div className="relative z-10 w-full flex h-full">
            ...
            {isMatchFinished ? (
              ...
              <>
                {/* 2. SCORE COLUMN (Removed border-l, added py-3) */}
                <div className="w-[360px] h-full flex flex-col justify-center px-10 py-3 bg-black/10 border-r border-white/10 shrink-0">
                  <div className="text-white text-xl font-black tracking-widest uppercase mt-2 drop-shadow-md">
                    {battingInitials} <span className="text-white/50 mx-2 text-lg">VS</span> {bowlingInitials}
                  </div>
                  <div className="flex items-baseline gap-4">
                    <span className="font-mono text-[5rem] font-black text-white leading-none drop-shadow-lg tracking-tighter">
                      {score}<span className="text-[3.5rem] text-white/80">/{wickets}</span>
                    </span>
                    <span className="font-bold text-2xl text-white/90 leading-none drop-shadow-md bg-black/10 px-3 py-1.5 rounded border border-white/10">
                      {displayOvers} <span className="text-lg font-normal text-white/60">Ov</span>
                    </span>
                  </div>
                  <div className="text-xs text-amber-400 font-bold uppercase tracking-wider truncate mt-1.5 drop-shadow-sm">
                    {scoreContextText}
                  </div>
                </div>

                {/* 3. BATSMEN COLUMN */}
                <div className="w-[400px] h-full flex flex-col justify-center px-8 bg-black/20 border-r border-white/10 shrink-0 text-white shadow-[10px_0_20px_rgba(0,0,0,0.2)]">
                  <div className={`flex justify-between items-end ${striker ? "font-bold" : "opacity-50"}`}>
                    <span className="truncate pr-3 flex items-center gap-2 text-2xl drop-shadow-md">
                      {striker || "Striker"}
                      {striker && <Zap size={18} className="text-amber-400 fill-amber-400 drop-shadow" />}
                    </span>
                ...
                <div className="flex-1 h-full flex flex-col justify-center px-8 bg-black/10 border-r border-white/10 overflow-hidden min-w-[300px]">
                  ...
                  <div className="flex items-center justify-start gap-2 overflow-hidden w-full">
                    {timeline.length === 0 ? (
                      ...
                      timeline.map((b, i) => {
                        ...
                        return (
                          <div key={i} className={`w-11 h-11 border-2 rounded-full shrink-0 flex items-center justify-center font-black ${bubbleClass} ${textSize} uppercase`}>
                            {text}
                          </div>
                        );
                      })
