...
export default function ScoreTicker({ match }) {
  ...
  // --- CHECK MATCH STATUS ---
  const isMatchFinished = match.status === "completed" || match.result;
  ...
  if (isMatchFinished) {
    if (match.result) {
      resultText = match.result; // Use DB result if available
    } else {
      ...
      if (match.currentInnings === 1) {
        if (score >= target) {
         ...
         resultText = `${battingTeam} score ${score}`;
      }
    }
  }
  ...
  return (
    <>
      ...
      <div className="w-full flex items-center justify-center font-sans px-10">
        {/* 1. LEFT TEAM LOGO */}
        <div className="w-24 h-24 flex-shrink-0 rounded-full bg-[#1C2128] border-4 border-[#1C2128] shadow-2xl z-20 flex items-center justify-center overflow-hidden relative -mr-7 anim-entry">
          <img
            src={leftLogo}
            alt="Team A"
        ...
        <div className="w-full max-w-[1800px] h-[130px] bg-[#1C2128] rounded-full border-[3px] border-teal-500/30 shadow-[0_10px_40px_rgba(0,0,0,0.6)] flex overflow-hidden relative z-10">
          ...
          {/* --- LEFT SECTION --- */}
          <div className="w-[30%] flex flex-col justify-center px-4 border-r border-white/10 bg-gradient-to-r from-[#161920] to-[#1C2128]">
            {!isMatchFinished ? (
              <>
                ...
                <div className="flex justify-between items-center mb-1">
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <span className="text-white font-bold text-2xl uppercase tracking-tight truncate max-w-[200px]">{striker}</span>
                  </div>
                  <div className="text-teal-400 font-mono font-bold text-3xl">{sStats.runs} <span className="text-slate-500 text-lg font-sans">({sStats.balls})</span></div>
                </div>
                <div className="flex justify-between items-center opacity-60">
                  <span className="text-slate-300 font-bold text-xl uppercase tracking-tight pl-5">{nonStriker}</span>
              ...
              // RESULT MODE: Innings 1 Score
              <div className="flex flex-col items-center">
                 <span className="text-slate-400 font-bold text-sm uppercase">{match.innings[0]?.battingTeam}</span>
                 <span className="text-white font-black text-4xl">{match.innings[0]?.score}/{match.innings[0]?.wickets}</span>
                 <span className="text-slate-500 font-mono text-sm">({match.innings[0]?.over}.{match.innings[0]?.overBallCount} Ov)</span>
              </div>
            )}
          </div>

          {/* --- CENTER: SCOREBOARD / RESULT --- */}
          <div className="flex-1 bg-[#0F1115] relative flex flex-col items-center justify-center px-4 overflow-hidden border-x border-teal-500/50">
            ...
            {!isMatchFinished ? (
              ...
              <>
                <div className="relative z-10 w-full flex items-center justify-between px-2">
                  <div className="flex flex-col items-end mr-4">
                    <span className="text-teal-400 font-black text-3xl uppercase tracking-widest leading-none">{battingTeam.substring(0, 3)}</span>
                    <span className="text-slate-600 font-bold text-sm uppercase">Batting</span>
                  </div>
                  <div className="bg-gradient-to-b from-teal-600 to-teal-800 text-white px-10 py-1 rounded-xl shadow-lg border border-teal-400/30">
                    <span className="font-black text-6xl tracking-tighter drop-shadow-md">{score}/{wickets}</span>
                  </div>
                  <div className="flex flex-col items-start ml-4">
                     <div className="bg-slate-800 text-white px-3 py-1 rounded-lg border border-slate-600 flex flex-col items-center min-w-[80px]">
                        ...
                        <span className="font-black text-3xl leading-none">{overs}</span>
                     </div>
                  </div>
                </div>
                <div className="relative z-10 mt-2">
                  {isChasing ? (
                    <div className="text-yellow-400 text-sm font-bold uppercase tracking-[0.2em]">Need <span className="text-white text-lg">{need}</span> off <span className="text-white text-lg">{ballsLeft}</span> balls</div>
                  ) : (
              ...
              <div className="flex flex-col items-center z-10">
                 <span className="text-yellow-400 font-black text-xl uppercase tracking-[0.3em] mb-1">MATCH FINISHED</span>
          ...
          {/* --- RIGHT SECTION --- */}
          <div className="w-[30%] flex flex-col justify-center px-4 border-l border-white/10 bg-gradient-to-l from-[#161920] to-[#1C2128]">
             {!isMatchFinished ? (
                // Live Bowler Stats
                <>
                  <div className="flex justify-between items-center mb-2 border-b border-white/5 pb-1">
                    <span className="text-white font-bold text-xl uppercase tracking-tight truncate max-w-[200px]">{bowler}</span>
                    <div className="text-right whitespace-nowrap">
                      <span className="text-yellow-400 font-mono font-bold text-2xl">{bStats.wickets}-{bStats.runs}</span>
                      <span className="text-slate-500 text-sm font-bold ml-2">{bOvers}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {timeline.map((b, i) => (
                      <div key={i} className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm border ${b.isWicket ? 'bg-red-600 border-red-400' : b.runs>=4 ? 'bg-teal-500 text-black border-teal-400' : 'bg-slate-800 border-slate-600 text-slate-400'}`}>
                        {b.isWicket ? 'W' : b.runs}
                      </div>
                    ))}
                  </div>
                ...
                // RESULT MODE: Innings 2 Score
                match.innings[1] ? (
                  <div className="flex flex-col items-center">
                    <span className="text-slate-400 font-bold text-sm uppercase">{match.innings[1]?.battingTeam}</span>
                    <span className="text-white font-black text-4xl">{match.innings[1]?.score}/{match.innings[1]?.wickets}</span>
                    <span className="text-slate-500 font-mono text-sm">({match.innings[1]?.over}.{match.innings[1]?.overBallCount} Ov)</span>
                  </div>
                ) : (
                   <div className="flex items-center justify-center h-full text-slate-500 uppercase font-bold text-sm">Innings Break</div>
                )
             )}
          </div>
        </div>
        ...
        {/* 3. RIGHT TEAM LOGO */}
        <div className="w-24 h-24 flex-shrink-0 rounded-full bg-[#1C2128] border-4 border-[#1C2128] shadow-2xl z-20 flex items-center justify-center overflow-hidden relative -ml-7 anim-entry">
          <img
            src={rightLogo}
            alt="Team B"
            className="w-full h-full object-cover"
